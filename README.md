...
- `sandbox_ttl_seconds=None`：不启用 SDK 的无操作续期，由 OpenSandbox 使用服务端默认生命周期策略。
...
`sandbox_ttl_seconds` 是 Sandbox 的无操作超时窗口：
...
- 不传或传 `None`：SDK 不向 OpenSandbox 传递 `timeout`，也不会主动续期，使用 OpenSandbox 自身默认值。
- 实际默认值由 OpenSandbox Service 的版本和服务端配置决定；如果服务端配置为 180 秒，未传参数时就按 180 秒执行。
- 传入正数：创建时设置 OpenSandbox `timeout=timedelta(seconds=...)`，并在每个用户级 Backend 操作完成后调用续期接口，将过期时间重置为“当前时间 + 该时长”。
- 不能传 `0` 或负数；如果希望永久保留，应使用 OpenSandbox 自身支持的明确配置方式，
  SDK 不会把 `None` 转换成 OpenSandbox 的“手动清理”语义。
- 如果 OpenSandbox 还配置了独立的空闲清理、资源回收或最大生命周期限制，最终删除时间由最先触发的限制决定；SDK 的 `sandbox_ttl_seconds` 不会覆盖这些独立限制。
